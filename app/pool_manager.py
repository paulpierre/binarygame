...
pool_manager
--
• Exports pool objects and saves them into the database
• Gets called on by cron to create pool

Functions:

    Pre-pool
    --------
    • Register current pool (cron will do this 3 times for upcoming pools)
        • Generate hash
        • Set time start
    ...
    Pool open
    ---------
    • Update state to accept users
    • Add user to pool
        • Update buy bids
        • Update sell bids
        • Update total amount
        • Update multipliers
    • If user leaves pool, credit their account minus pool fee
    • Create transactions pointing to pool_id
        • Credit / debit user account, verify amounts w/ integrity hash

    Pool start
    ----------
    • Lock pool state
    • Update price every 15s
    ...
    Pool close
    ----------
    • Redistribute tokens based on multipliers
    • Create transactions pointing to user_ids
        • Debit the pool, verify amounts w/ integrity hash
    • Write database object
"""
...
pool_hasher = Hashids(min_length=7,alphabet='1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ',salt=config.app.config['SECRET_KEY'])
